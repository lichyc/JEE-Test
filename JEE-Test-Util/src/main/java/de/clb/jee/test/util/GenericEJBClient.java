...
/**
 * See https://docs.jboss.org/author/display/AS71/EJB+invocations+from+a+remote+
 * client+using+JNDI for more information.
 * 
 * @author clichybi
 */
...
//    
	 ...
	 *            application.xml of the EJB deployment on the server. Since we
	 *            haven't deployed the application as a .ear, the app name for
	 *            us will be an empty string
	 * @param moduleName
	 *            This is the module name of the deployed EJBs on the server.
	 *            This is typically the jar name of the EJB deployment, without
	 *            the .jar suffix, but can be overridden via the ejb-jar.xml In
	 *            this example, we have deployed the EJBs in a
	 *            jboss-as-ejb-remote-app.jar, so the module name is
	 *            jboss-as-ejb-remote-app
	 * @param distinctName
	 *            AS7 allows each deployment to have an (optional) distinct
	 *            name. We haven't specified a distinct name for our EJB
	 *            deployment, so this is an empty string
	 * @param beanName
	 *            The EJB name which by default is the simple class name of the
	 *            bean implementation class final String beanName =
	 *            SimpleSsb.class.getSimpleName();
	 * @param viewClassName
	 *            the remote view fully qualified class name
	 * @return
	 * @throws NamingException
	 */
	public static Object lookupRemoteStatelessSSB31(Properties properties, String appName, String moduleName, String distinctName, String beanName, String viewClassName) throws NamingException {
		// forward
		return GenericEJBClient.lookupRemoteStatelessSSB31(properties, "ejb:" + appName + "/" + moduleName + "/" + distinctName + "/" + beanName + "!" + viewClassName);
	}
	...
	public static Object lookupRemoteStatelessSSB31(String appName, String moduleName, String distinctName, String beanName, String viewClassName) throws NamingException {
		// forward
		return GenericEJBClient.lookupRemoteStatelessSSB31(new Properties(),appName, moduleName, distinctName, beanName, viewClassName);
	}
	...
	public static Object lookupRemoteStatelessSSB31(Properties properties, String name) throws NamingException {
		Properties env = properties == null ? new Properties() : properties;
